...
{
    var graphWrapperID = 'graphwrapper_' + newGraphDivID;
    var rowLegendName = 'graphrowlegend_' + newGraphDivID;
    ...
    parentDiv.append('<div id="' + graphWrapperID + '" style="width=100%;"></div>');
    var graphWrapperDiv = $('#' + graphWrapperID);
    //var rowString = '<div id="'+ newGraphDivID +'_header" class="timelineRow timelineRowHeader">jkl</div>';
    //newGraphDiv.append(rowString);

    //graphWrapperDiv.append('<div id="' + rowLegendName + '" style="text-align:center;float: left; display:inline-block;background-color:#eee;">Work<br>Conveyor View<br>Missed Window<br>Zone Interruption<br>Unknown Container<br>something<br>something else</div>');
    graphWrapperDiv.append('<div id="' + rowLegendName + '" style="text-align:center;float: left; border: 1px solid black; border-radius: 5px; border-right: 0px;display:inline-block;background-color:#eee;"></div>');
    ...
    for (r=0; r<graphStruct.rowStruct.length; r++)
    {
        ...
        var legendRowID = 'legendrow_' + rowID;
        var legendRowString = '<div id="' + legendRowID +'" class="timelineRow '+ rowClass +'"';
        legendRowString += ' style="';
        rowString += ' width: 100%;';
        legendRowString += ' height: ' + rowHeight + ';';
        legendRowString += ' line-height: ' + rowHeight + ';';        
        if (graphStruct.rowStruct[r].rowColor != null)
            legendRowString += ' background-color: ' + graphStruct.rowStruct[r].rowColor + ';'
        legendRowString += '">'
        if (graphStruct.rowStruct[r].title != null)
            legendRowString += graphStruct.rowStruct[r].title;
        legendRowString += '</div>';
        console.log(legendRowString);
        
        legendDiv.append(legendRowString);
...
function addTimelineBars(graphID, rowID, barData)
{
    ...
    if (rowDiv.length === 0 || graphDiv.length === 0) {
    ...
    for (b=0; b<barData.length; b++)
    {
        ...
        if (barEnd < barStart) {                        // If start/end are reversed, swap them
            [barStart, barEnd] = [barEnd, barStart];
            barLen = -barLen;
        }
        if (barEnd <= 0 || barStart >= 100)     // If bar isn't inside of timeline, don't add it
            continue;
        ...
        // style
        barString += ' style="overflow:hidden; opacity: '+ opacityTracker +';top: 10%; height: ' + overlapHeightTracker + '%; width: '+ barLen +'%; margin: 0 '+ barStart +'%; background-color: '+ barColor +';';
        ...
        barString += '">';
        //if (barData[b].title != null)
        //    barString += barData[b].title;
        barString += '</div>';
...
require(['dojo/domReady!'], function() {
    ...
    var graphStructure =
    {
        ...
        rowStruct: [
            {id:"row1", title: "Work", rowColor:"#aaaaaa", barColor:"#3d3"},
            {id:"row2", title: "WUI Exception View", barColor:"#33d"},
            {id:"row3", title: "Missed Window", barColor:"#dd3"},
            {id:"row4", title: "Unknown Ctn", barColor:"#3ac"},
            {id:"row5", title: "Duplicate Scan", barColor:"#d33"},
            {id:"row6", title: "Interrupted Zone", barColor:"#33F"},
    ...
    row1data = [
        { id:"bar0", title:"[BWS02 - Zone 3]sldldsjflskjflskjfldskfj", start:-15, end:17 },
    ...
    row2data = [
        ...
        { id:"2bar3", start:80, end:93, barColor:"#fa4"},
    ];
    row3data = [
